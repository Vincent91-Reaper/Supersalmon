    ...
    async def create_soup(self, url, params=None):
        ...
        for cc in get_tidal_regions_to_fetch():
            try:
                ...
                # Check if album has artist info at album level
                if "artists" in data:
                    click.secho(f"DEBUG TIDAL API: Album-level artists: {data.get('artists', [])}", fg="yellow")
                if "artist" in data:
                    click.secho(f"DEBUG TIDAL API: Album-level artist: {data.get('artist', {})}", fg="yellow")
                ...
                if tracklist.get("items"):
                    ...
                    click.secho(f"DEBUG TIDAL API: WARNING - No tracks in tracklist!", fg="red")
                    click.secho(f"DEBUG TIDAL API: This may be due to regional restrictions for region {cc}", fg="red")
                    # Continue to try next region instead of failing here
                    if cc != get_tidal_regions_to_fetch()[-1]:
                        click.secho(f"DEBUG TIDAL API: Trying next region...", fg="yellow")
                        continue
